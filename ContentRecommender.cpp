...
void ContentRecommender::build_utility_matrix() {
    ...
    utility_matrix = alloc2dArray<float>(row_count, col_count);
...
float *ContentRecommender::create_representation(set<string> &terms, vector<string> &hits, vector<float> &idf) {
    float *representation = new float[terms.size()];
    for (auto hit : hits) {
...
void ContentRecommender::build_representations() {
    ...
    items_representation = alloc2dArray<float>(item_contents.size(), unique_main_terms.size());
...
void ContentRecommender::do_content_predictions(float **items_representations,
                                                float **users_representations) {
    size_t missing = 0;
    ...
    for (int idx = 0; idx < items_representations_size; idx++) {
        items_vectors_norms[idx] = vector_norm(items_representations[idx], unique_main_terms.size());
    }

    for (int idx = 0; idx < users_representations_size; idx++) {
        users_vectors_norms[idx] = vector_norm(users_representations[idx], unique_main_terms.size());
    }
...
float **ContentRecommender::compute_users_representations(float **items_representations, size_t term_count) {
    float **users_representations = alloc2dArray<float>(target_users.size(), term_count);
...
void ContentRecommender::compute_users_factors_matrix() {
    ...
    users_representation = compute_users_representations(items_representation, unique_main_terms.size());
}

void ContentRecommender::clear_utility_matrix() {
    free2DArray(items_representation, items.size());
    free2DArray(users_representation, target_users.size());
    free2DArray(utility_matrix, users.size());
